...
var ValidatorMixin = {
  ...
  resolveToParse: function (xpath, typeName) {
    ...
    if (!type) {
      ...
      if (_.isArray(type.base)) {
        var res = [];
        _.each(type.base, function (base) {
          res = res.concat(self.resolveToParse(xpath, base));
        });
        return res;
      }
      else {
        return self.resolveToParse(xpath, type.base);
      }
    }
    else {
      return [];
  ...
  flattenAttributes: function (xpath, type, newValue) {
    var self = this;
    
    var value = _.clone(newValue);
    
    if (value[self.attrkey]) {
      _.each(value[self.attrkey], function(child, name) {
        value[name] = child;
      });
      delete value[self.attrkey];
    }
  ...
  nodeNamespace: function (node) {
    ...
    if (typeof(node[self.xmlnskey].uri) == "undefined") {
  ...
  namespacedName: function (namespaces, defaultNamespace, name) {
    ...
    else if (typeof(defaultNamespace) != "undefined") {
...
function validator(xpath, currentValue, newValue, stack) {
  ...
  if (options.flattenAttributes)
    newValue = parser.flattenAttributes(xpath, type, newValue);
